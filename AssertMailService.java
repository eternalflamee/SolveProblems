...
public class AssertMailService {

    public static void main(String[] args) {
        // Random variables
        String randomFrom = "Багратион оглянул свою свиту своими большими, ничего не выражающими."; // Некоторая случайная строка. Можете выбрать ее самостоятельно.
        String randomTo = "Сменившись из цепи, Ростов успел соснуть несколько часов перед утром и чувствовал себя веселым";  // Некоторая случайная строка. Можете выбрать ее самостоятельно.
        int randomSalary = 100;  // Некоторое случайное целое положительное число. Можете выбрать его самостоятельно.
...
//   где по получателю можно получить список зарплат, которые были ему отправлены.
    ...
    public static class MailService<T> implements Consumer<Sendable<T>> {
        public Map<String, List<T>> mailBox;
        public MailService() {
            mailBox = myHashMap;
        }

        HashMap<String, List<T>> myHashMap = new HashMap() {
            @Override
        ...
        public void accept(Sendable<T> t) {
            mailBox.computeIfAbsent(t.getTo(), k -> new ArrayList<>()).add(t.getContent());
        }
